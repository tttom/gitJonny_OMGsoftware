...
%         fileName='C:\Users\Jonathan Nylk\Desktop\DeconvolutionMasterclass_MSquared_02-12-2015\Example Beads\ex3\recording0_lambda532nm_alpha-7_beta100.mat';
        fileName='E:\2015-05-29_BeadTest\Aperture_09\2015-05-29 12_45_09.800\recording0_lambda532nm_alpha7_beta100.mat';
        load(fileName,'recordedImageStack','xRange','yRange','zRange');
    end
    
    recordedImageStack=recordedImageStack(:,floor(length(yRange)/4):floor(length(yRange)*3/4),:);
    yRange=yRange(floor(length(yRange)/4):floor(length(yRange)*3/4));
...
%             dataStruct(n).analysisRangeZ=[max(1,dataStruct(n).maxZCoord-20),min(length(zRange),dataStruct(n).maxZCoord+5)];
            dataStruct(n).analysisRangeZ=[max(1,dataStruct(n).maxZCoord-5),min(length(zRange),dataStruct(n).maxZCoord+20)];
        end
    end
