...
%Set default parameters
    ...
    %Built-in function VideoReader creates an object that can read in a
    %multimedia file, and the designated .avi file is read into it.
	if (exist('VideoReader','class'))
        ...
        for frameIndexIdx = 1:length(frameIndexes)
            ...
            if (colorEncodingFor24bits)
                ...
                img = single(vidFrame(:,:,3))+single(vidFrame(:,:,2))*(2^8)+single(vidFrame(:,:,1))*(2^16);
            else
            ...
            if (isempty(projectionDimensionOrSubCube))
                ...
                %Project or crop the data cube if a projection
                %dimension/region was specified as an input
        ...
        %Alternate processing method using the old MATLAB function aviread
        warning off;
