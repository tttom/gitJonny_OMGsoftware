...
function [psf psfTwoPhotonSwiped psfTwoPhotonCylindricalLens]=calcLightSheetPsf(xRange,yRange,zRange,tilt,excitation,modulation,refractiveIndexOfSample)
    ...
    if isfield(cache,key)
        ...
        stopWatch=tic();
        psf=calcVectorialPsf(xRangeForProj,zRange,yRange,excitation.wavelength,@(U,V) pupilFunctor(U,V)/sqrt(2),@(U,V) 1i*pupilFunctor(U,V)/sqrt(2),numericalAperture,refractiveIndexOfSample,projectionDimension);
    ...
    if (nargout>=2)
        ...
        if (isfield(cache,keyTwoPhoton))
            ...
            stopWatch=tic();
            [psfTwoPhoton,~,psfTwoPhotonSwiped]=calcVectorialPsf(xRangeForProj,zRange,yRange,2*excitation.wavelength,@(U,V) pupilFunctor(U,V)/sqrt(2),@(U,V) 1i*pupilFunctor(U,V)/sqrt(2),numericalAperture,refractiveIndexOfSample,projectionDimension);
            if (nargout>=3)
