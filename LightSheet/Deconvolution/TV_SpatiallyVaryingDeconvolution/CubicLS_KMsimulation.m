function CubicLS_KMsimulation(experimentFilePath,excNA,LSalpha,detDiam,PMalpha,yRes,PMchoice)
%Inputs: (1) experimentFilePath is the path to an avi file, which is taken
%in and used to define the .json file and output path; (2) excNA is the
%numerical aperture of the illumination, which is between 0.4 and 0.8 on
%our system. Typically fixed at 0.42; (3) LSalpha is the strength of the
%light sheet, between -7 and -20 on our system; (4) detDiam is the
%diameter--not numerical aperture--of the detection pinhole. 3.94e-3 m
%corresponds to detection NA = 0.42, and 7.5e-3 is NA = 0.8; (5) PMalpha is
%the strength of the phase mask; (6) yRes is the number of PSFs to
%simulate. Standard conditions are excNA = 0.42, yRes = 9, and LSalpha,
%PMalpha, and detDiam are my variable parameters.

%Outputs: a fig file containing projections of the detection PSFs, light
%sheet PSF, combined PSF, and deconvolved PSF; and a mat file containing
%the deconvolved PSF ('restoredTiledPsf') only.

%Note that the pupil function (2D Cubic or General Cubic) needs to be
%changed by editing lines 96 and 97.
    ...
    config.excitation.fractionOfNumericalApertureUsed = excNA/config.excitation.objective.numericalAperture;
    config.modulation.alpha = LSalpha;
    ...
    config.detection.objective.irisDiameter = detDiam; % in meters
    ...
    config.detection.mask.alpha = PMalpha; % in wavelengths, over its diameter (i.e. not considering the iris here)
...
%     config.detection.mask.beta=0; % use this for the classic cubic
    ...
    lightSheetOffset = [0 0 0]*1e-6; % in case of no misalignment between the light sheet focus and the center of the FOV (x,y) and focus (z).
    shearFactors = [0 0]; % it appears that the stage translates at an angle slightly non-orthogonal. That is ok as long as it doesn't change
    viewSize=[128 2048 500]; % in pixels, a subset makes processing faster and doesn't overload the output image
%     viewSize=[128 256 128]; %[256, 128, 256]; % in pixels, a subset makes processing faster and doesn't overload the output image
    ...
    % effects. This should be chosen wisely as it also defines in how many sections the 
    ...
    nbPsfs = [1 yRes 1]; % spatially variant, 15 different psfs along the light sheet propagation axis
%     nbPsfs = [1 1 1] is the spatially invariant case.
...
psfSize = floor(viewSize./nbPsfs); 
    ...
    pupilFunctionGen = @(U,V) exp(2i*pi*(detectionAlpha*((U*cRA-V*sRA).^3+(U*sRA+V*cRA).^3)+detectionBeta*((U*cRA-V*sRA).^2.*(U*sRA+V*cRA)+(U*cRA-V*sRA).*(U*sRA+V*cRA).^2)));
    ...
    elseif PMchoice == 2
        pupilFunction = pupilFunctionGen;
    end
...
%     %Added to test whether star shape is due to use of full LS 'PSF'
%     lightSheetPsfDirac=zeros(size(lightSheetPsf));
%     for i=1:yRes
%         lightSheetPsfDirac(1,psfSize(2)*(i-1)+psfSize(2)/2,:)=lightSheetPsf(1,psfSize(2)*(i-1)+psfSize(2)/2,:);
%     end
%     %End addition
    ...
    %[psfs, tiledPsfs] = calcCombinedPsf(lightSheetPsfDirac,detectionPsfs,nbPsfs,dataSize,pixelPitchInSample,shearFactors);
    
    % Remove the distortion
    function img = straightenAiryBend(img,yRange,zRange)
    ...
    for psfIdx=1:prod(nbPsfs),
        ...
        psfs(:,:,:,psfIdx) = straightenAiryBend(psfs(:,:,:,psfIdx),...
            yRange(cellCenter(2))+yRangePsfCalc*0,zRange(cellCenter(3))+zRangePsfCalc); % *0: shift each psf as a block
    end
...
%     squeeze(real(sum(sum(sum(kernels))))).'./squeeze(real(sum(sum(sum(psfs))))).'
    ...
    function restoredDataCube = spVarDeconvolution(recordedImageStack)
        restoredDataCube = spatiallyVariantConvolution(recordedImageStack,kernels);
        restoredDataCube = real(restoredDataCube);
        restoredDataCube = straightenAiryBend(restoredDataCube,yRange,zRange); % straighten result to undo the psf alignment
    end
...
%      restoredDataCube = restoreData(recordedImageStack);
     logMessage('Deconvolving the PSFs too...');
     restoredTiledPsfs = restoreData(tiledPsfs);
    
    if nargout<1,
        logMessage('Displaying...');
        close all;
        
%         figs(1) = displayMultiView(xRange,yRange,zRange,{'detection','lightsheet','system'},...
%              detectionPsfs,lightSheetPsf,tiledPsfs);
        
        %figs = displayMultiView(xRange,yRange,zRange,{'detection','lightsheet'},detectionPsfs,lightSheetPsfDirac);
        %figs = displayMultiView(xRange,yRange,zRange,{'detection','lightsheet'},detectionPsfs,lightSheetPsf);
         figs = displayMultiView(xRange,yRange,zRange,{'detection','lightsheet','system','deconvolved'},...
...
%             tiledPsfs,recordedImageStack,restoredDataCube);
         ...
         outputFilePath = strcat(outputFilePath(1:end-4),sprintf('_%d_%d_%d_%d_%d.mat',[excNA LSalpha detDiam PMalpha yRes]));
         outputFigurePath = strcat(outputFilePath(1:end-4),'.fig');
         logMessage('Saving figure to %s...',outputFigurePath);
         saveas(figs,outputFigurePath);
        
        %clear restoredDataCube;
    end
     logMessage('Saving to %s...',outputFilePath);
     save(outputFilePath,'detectionPsfs','lightSheetPsf','xRange','yRange','zRange');
     save(outputFilePath,'restoredTiledPsfs','xRange','yRange','zRange');
