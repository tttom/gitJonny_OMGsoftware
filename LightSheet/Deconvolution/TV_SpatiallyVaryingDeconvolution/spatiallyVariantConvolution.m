...
% Leave this note in if reusing this code or parts of it. Thanks.
%
%
% Example:
%       nbKernels=[4 6];
...
%         data=ones(128,64);
    ...
    if nargout<1,
        ...
        figure();
        axs(1)=subplot(1,3,1);
        imagesc(data); axis image; title('input');
        hold on; scatter(YCellPos(:),XCellPos(:),'filled'); colorbar();
        axs(2)=subplot(1,3,2);
        imagesc(convolved); axis image; title('convolved');
        hold on; scatter(YCellPos(:),XCellPos(:),'filled'); colorbar();
        axs(3)=subplot(1,3,3);
        imagesc(convolved-data); axis image; title('convolved-data');
        hold on; scatter(YCellPos(:),XCellPos(:),'filled'); colorbar();
        linkaxes(axs);
