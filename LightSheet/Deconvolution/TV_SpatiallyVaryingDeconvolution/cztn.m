...
% Situates the built-in MATLAB function czt (chirp z transform) to
% calculate a multidimensional transform to retrieve the partial spectrum of
% matrix input x.
...
% remains the same.
    ...
    %Manual definition of arguments
    ...
    %Call the built-in function czt (chirp z transform) to calculate the
    %spectrum
    f=x;
    for dimIdx=1:nbDims
        if (size(f,dimIdx)>1)
