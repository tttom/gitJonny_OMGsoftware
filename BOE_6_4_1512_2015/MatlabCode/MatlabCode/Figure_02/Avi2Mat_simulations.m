...
DataFolder          = '3um73mW';             %Folder containing the data files
FOLDER              = ['C:\Users\Jonathan Nylk\Documents\GitHub\gitJonny_OMGsoftware\BOE_6_4_1512_2015\Fig2_3um73mW', filesep, DataFolder];              	%Full path to folder containing the data files
...
for movnr = 1:size(FILES,1) %17 used in fiber trap paper
    %% Loading movie
%     inputFile           = [FOLDER,'\',FILES.name(movnr),'.avi'];
%     outputFile          = [FOLDER,'\',FILES.name(movnr),'.mat'];
    inputFile = 'C:\Users\Jonathan Nylk\Documents\GitHub\gitJonny_OMGsoftware\BOE_6_4_1512_2015\Fig2_3um73mW\3um73mW\3um_73mW_a.avi';
    outputFile = 'C:\Users\Jonathan Nylk\Documents\GitHub\gitJonny_OMGsoftware\BOE_6_4_1512_2015\Fig2_3um73mW\3um73mW\3um_73mW_a.mat';
    inputVideo          = VideoReader(inputFile);
    nFrames             = inputVideo.NumberOfFrames;
    rFrames             = 2:nFrames;
    rFrames = [2:10001]; % override to save time
...
%                                             + (yCoords - (imWidth / 2) - wobbleStrength * (rand(1) - 0.5)).^2) / sqrt(2) / 8).^4);
%                                             % Insane multiple multiscale objects target
%                                         %    noiseFreeImageArray(:,:,idx) = exp(-1 .* (sqrt((xCoords - (imWidth / 2) - wobbleStrength * (rand(1) - 0.5)).^2 ...
%                                         %    + (yCoords - (imWidth / 2) - wobbleStrength * (rand(1) - 0.5)).^2) / sqrt(2) / 6).^4)...
%                                         %    + exp(-1 .* (sqrt((xCoords - (imWidth / 4) - wobbleStrength * (rand(1) - 0.5)).^2 ...
%                                         %    + (yCoords - (imWidth / 4) - wobbleStrength * (rand(1) - 0.5)).^2) / sqrt(2) / 2).^20);
%                                         end
%                                         imageArray = noiseFreeImageArray + rand(size(noiseFreeImageArray));
%                                         imageArray = imageArray / max(imageArray(:));
%                                         mov0 = imageArray; clear imageArray;
...
%             figure,imagesc(me1(1+B:end-B,1+B:end-B));axis image;colormap gray;viscircles(center(1,:), radius(1),'EdgeColor','g');
        ...
        ROIr                        = 1:60;%510-imSize/2:510+imSize/2-1;
        ROIc                        = 1:60;%401-imSize/2:401+imSize/2-1;
...
%     [movOut,E,PP] = function_noiseRemoval_v1(mov2, 20, false);
    if 1
        [mov3,E,PP,val0]                 = function_noiseRemoval(mov2, 20,2:3); % old 4um: (mov2, 20,[1:7,10:11])
...
%     saveas(h,[FOLDER,'\',FILES{movnr,:}],'png') 
    ...
    figure;plot(PCA_Strength);

    PPall(:,movnr) = PP;
end
% save('PPall_3um','PPall','-v7.3')%,'p')

figure();
%     for idx = 1:size(mov3,3)
    for idx = 1:500
        subplot(1,2,1);
        imagesc(squeeze(mov2(:,:,idx)));
        axis image;
        title(num2str(idx));
        subplot(1,2,2);
        imagesc(squeeze(mov3(:,:,idx)));
        axis image;
        title(num2str(idx));
        drawnow;shg;
    end
