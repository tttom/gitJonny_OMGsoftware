...
DataFolder                  = 'Jonny and Claire - Data\2014_05_22\800nmGRINlens\4um';             %Folder containing the data files
...
%% * Loading DATA
load([FOLDER,filesep,'DATAC_F1_4um'])
...
d                           = 4.17e-6;                                  %Bead diameter [1.90 3.00 4.17]*1e-6
sdtd                        = 3e-8;                                     %Bead diameter standard deviation [0.02 0.02 0.03]*1e-6 
r                           = d/2;                                      %Bead radius
...
E                           = 3e-19;                                 	%Level of white background noise in the power spectrum
D                           = Dcalc;%3.8259e-14;%1e-13;                                    %Einstein's diffusion constant
gamma                       = kB*T/D;                                   %
...
psdFitR                             = fit(f(ind),MeanR(ind),psdft,psdfo,'problem',{W,0.5,250});
psdFitC                             = fit(f(ind),MeanC(ind),psdft,psdfo,'problem',{W,0.5,250});
...
% Finding peaks
[pksR,locsR]                        = findpeaks(MeanRsmooth,'THRESHOLD',0.005,'MINPEAKDISTANCE',100,'MINPEAKHEIGHT',1.5);
[pksC,locsC]                        = findpeaks(MeanCsmooth,'THRESHOLD',0.005,'MINPEAKDISTANCE',100,'MINPEAKHEIGHT',1.9);
...
meanPrefitE                         = mean([RprefitE(10:end),CprefitE(10:end)]);
meanPrefitD                         = mean([RprefitD(1:10),CprefitD(1:10)]);
...
for ID = 1:30       
    ...
    for k = 1:2
        if k == 1
            ...
            % Start points
            fParR                               = [fcR(ceil(ID/5)), 0, meanPrefitD];%     fParR                               = [fcR(ceil(ID/5)), 0, 6.2124e+04];
            fParC                               = [fcC(ceil(ID/5)), 0, meanPrefitD];%     fParC                               = [fcC(ceil(ID/5)), 0, 6.1082e+04];
...
% display([mean(fitdataR(2,1:10)), mean(fitdataC(2,1:10))])
...
% subplot(122),plot(fitdataR(2,:)/M^2),hold on,plot(fitdataC(2,:)/M^2,'r')
    ...
    c_rPSD                              = fit(DATA.P(1:25)',DATA.Results.DFT.RkappaPSD(1:25)',ft,fo);
    c_cPSD                              = fit(DATA.P(1:25)',DATA.Results.DFT.CkappaPSD(1:25)',ft,fo);
...
%             plot(c_rPSD,'k'), hold on,legend off
    ...
    c_cPSD4um       = c_cPSD;
    c_rPSD4um       = c_rPSD;
    stdkappaR4um    = stdkappaR;
    stdkappaC4um    = stdkappaC;
    KAPPAr4um       = DATA.Results.DFT.RkappaPSDMean(1,:);
    KAPPAc4um       = DATA.Results.DFT.CkappaPSDMean(1,:);
    left = 5; bottom = 5; width = 21; height = 10;
    fig = figure(101);
    set(fig,'units','centimeters ','Position', [left, bottom, width, height])
    subplot(121),errorbar(P,KAPPAc4um,1e6*stdkappaC,'o','linewidth',LW,'MarkerSize',MS), hold on, plot([0,1.1*max(P)], [0,1.1*max(P)]*c_cPSD.a,'k','linewidth',LW), title('x-axis','FontSize',FS), legend off
...
%         text(0.1*DATA.P(end),120,sprintf('{\\it\\kappa}_D = {\\ita}*{\\itP}, {\\ita} = %1.3f pN/(um*mW)',c_cPSD.a))
    subplot(122),errorbar(P,KAPPAr4um,1e6*stdkappaR,'o','linewidth',LW,'MarkerSize',MS), hold on, plot([0,1.1*max(P)], [0,1.1*max(P)]*c_rPSD.a,'k','linewidth',LW),title('y-axis','FontSize',FS), legend off
        xlim([0,1.1*max(P)]),ylim([0,150]),xlabel('power, {\itP} [mW]','FontSize',FS),ylabel('force constant, {\it\kappa} [pN/um]','FontSize',FS)
...
%% Saving
save('PaperData4um','KAPPAr4um','KAPPAc4um','stdkappaR4um','stdkappaC4um','c_rPSD4um','c_cPSD4um')
