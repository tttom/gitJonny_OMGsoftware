function[FILES DIR pressure] = filenames(FOLDER,TYPE,SIZE,p)
%MVGK mar. 2013
...
for i = 1:size(list,1)
    dateModified(i)         = 0;%60^2*hour(list(i).date)+60*minute(list(i).date)+second(list(i).date);
    ...
    if size(fileName{k},2) > 3;
        if strcmp(fileName{k}(end-length(TYPE)+1:end), TYPE);
            if fileSize(k) > SIZE(1) && fileSize(k) < SIZE(2)
                kk              = kk+1;
                FILES.name{kk,:} 	= fileName{k}(1:end-4);
                if p
                    if k-1<1
                        ind = k+1;
...
% FILES                       = sortrows(struct2dataset(FILES));
end
