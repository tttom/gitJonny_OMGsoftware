function [output Greg CC CCdft] = dftregistration(buf1ft,buf2ft,usfac)
...
% Partial-pixel shift
    ...
    if usfac > 2,
        ...
        CCdft       = conj(dftups(buf2ft.*conj(buf1ft),ceil(usfac*1.5),ceil(usfac*1.5),usfac,...
                      dftshift-row_shift*usfac,dftshift-col_shift*usfac))/(md2*nd2*usfac^2);
        % Locate maximum and map back to original pixel grid 
        [max1,loc1] = max(CCdft);   
        [max2,loc2] = max(max1); 
        rloc        = loc1(loc2); cloc = loc2;
        CCmax       = CCdft(rloc,cloc);
